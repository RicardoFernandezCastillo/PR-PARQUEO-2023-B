...
Stream<QuerySnapshot> getSpaces(DataReservationSearch dataSearch) async* {
  ...
  final QuerySnapshot pisoQuerySnapshot = await getCollectionSnapshot(
      dataSearch.idParqueo.collection(SubCollectionParking.pisos));

  for (QueryDocumentSnapshot pisoDocument in pisoQuerySnapshot.docs) {
    final Map<String, dynamic> pisoData =
        pisoDocument.data() as Map<String, dynamic>;

    final QuerySnapshot filaQuerySnapshot = await getCollectionSnapshot(
        dataSearch.idParqueo
            .collection(SubCollectionParking.pisos)
            .doc(pisoDocument.id)
            .collection(SubCollectionParking.filas));

    for (QueryDocumentSnapshot filaDocument in filaQuerySnapshot.docs) {
      final Map<String, dynamic> filaData =
          filaDocument.data() as Map<String, dynamic>;
